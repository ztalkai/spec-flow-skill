...
只加载 `SHUORENHUA.md` 时，也必须能完成基础改写。下面这些规则默认直接生效：
...
单文件模式只是兜底，不是完整模式。只要环境里能读 `references/`，默认就继续补看对应文件；只有在 system prompt 真的只给了 `SHUORENHUA.md` 时，才退化为只按本文件做基础清理。
...
- 本文件可以单独兜底；完整模式默认是 `SHUORENHUA.md` + `references/` 一起工作
